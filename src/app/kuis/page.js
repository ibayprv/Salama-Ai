...
import { Award, Timer, ArrowRight, RefreshCw, CheckCircle, XCircle, ShieldCheck, Zap } from 'lucide-react';
...
export default function Kuis() {
  ...
  // Generate 10 random questions dynamically from seedWords
  const startQuiz = () => {
    // 1. Filter words by language selection
    let pool = seedWords;
    if (quizLanguage !== 'campuran') {
      pool = seedWords.filter(w => w.bahasa.toLowerCase() === quizLanguage);
    }
        ...
        <div className="p-8 sm:p-12 rounded-3xl glass-panel border border-white/10 text-center space-y-8 relative overflow-hidden">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-400 max-w-md mx-auto leading-relaxed">
              Uji kemampuan bahasa Ternate dan Sula Anda secara seru! Jawab 10 soal kosakata pilihan ganda dalam batas waktu 15 detik per soal.
            </p>
          </div>
        ...
        <div className="p-6 sm:p-8 rounded-3xl glass-panel border border-white/10 space-y-6 relative">
          ...
          <div className="flex justify-between items-center pb-4 border-b border-white/5">
            <div>
              ...
              <span className="text-lg font-bold text-white">Soal {currentIdx + 1} <span className="text-xs text-slate-500">dari 10</span></span>
            </div>
          ...
          {hasAnswered && (
            <div className="flex justify-end pt-4 border-t border-white/5">
              ...
              >
                <span>{currentIdx < 9 ? 'Soal Berikutnya' : 'Selesai & Lihat Skor'}</span>
        ...
        <div className="p-8 sm:p-12 rounded-3xl glass-panel border border-white/10 space-y-8">
          ...
          <div className="text-center space-y-4">
            ...
            <div className="py-6 inline-flex flex-col items-center justify-center p-8 bg-slate-950/80 border border-white/10 rounded-2xl shadow-inner min-w-[200px]">
              ...
              <span className="text-5xl font-black text-gold-500 mt-2">{score} <span className="text-xl text-slate-500 font-medium">/ 100</span></span>
              <span className="text-xs font-semibold text-emerald-500 mt-2 flex items-center">
                <CheckCircle className="h-3 w-3 mr-1" /> {correctCount} Soal Benar
              </span>
            </div>
          </div>
